plot4<-function()
{
  ...
  with(data1,plot(Time_min,Global_active_power,type="l",xlim=c(0,x_m),xaxt = "n",xlab="",ylab="Global Active Power"))
  axis(side=1,at=labels.at,labels=labels)
  
  with(data1,plot(Time_min,Voltage,type="l",xlim=c(0,x_m),xaxt = "n",xlab="datetime",ylab="Voltage"))
  axis(side=1,at=labels.at,labels=labels)
  ...
  axis(side=1,at=labels.at,labels=labels)
  legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("black","red","blue"),bty = "n",lwd=2, cex=1, horiz = FALSE)
  
  
  with(data1,plot(Time_min,Global_reactive_power,type="l",xlim=c(0,x_m),xaxt = "n",xlab="datetime",ylab="Global_reactive_power"))
  axis(side=1,at=labels.at,labels=labels)
  dev.off()
}
